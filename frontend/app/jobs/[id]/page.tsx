...
export default function JobsPage() {
  ...
  const handleMatch = async (jobId: number) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || 'Match failed — make sure you have scored a resume first')
    } finally { 
      setMatching(null) 
    }
  }
          ...
          {jobs.map(job => (
            <div key={job.id} className="card flex flex-col md:flex-row md:items-start gap-4">
              ...
              <div className="flex flex-col gap-2 md:min-w-32">
                ...
                >
                  <Zap size={14} />{matching === job.id ? 'Matching...' : 'Match Me'}
                </button>
                <span className="text-xs text-slate-500 text-center">{new Date(job.created_at).toLocaleDateString()}</span>
              </div>
